...
import customerPariplot from '../images/customerPairplot.png';
import commerceHeapMap from '../images/commerceHeatMap.png';
import linearModel from '../images/linearModel.png';
import linearEval from '../images/linearEval.png';

export default function Bike() {
  return (<Container>
  <BigTitle>Analysis and applying linear regression on customers' data</BigTitle>
  <DefaultDiv>
    The primary objective of this project is to make a linear regression model on customers' spend amount based on provided data.
  </DefaultDiv>
  <SemiTitle>
    This project uses data from Kaggle
  </SemiTitle>
  <div>
    <a target="_blank" href="https://www.kaggle.com/datasets/kolawale/focusing-on-mobile-app-or-website">
  ...
  <MenuDiv>
    ...
    <div>- Average session length: averate time spent per each session(mins)</div>
    <div>- Time on app: the time customer spent on the application</div>
    <div>- Time on website: the time customer spent on the website(mins)</div>
  ...
  <MenuDiv>
    - Pandas, Numpy, Scipy, Seaborn and Sklearn
  </MenuDiv>
  <SemiTitle>
    Data analysis & visualization
  </SemiTitle>
  
  <NumberTitle> 1. Visualize paired plots of numerical variables.</NumberTitle>
  <CodeBlock language="python" text={pairPlotCode}/>
  <ChartImg src={customerPariplot} style={{ height: 800 }}/>
  <DefaultDiv>
    <div>- Remove useless data for pair plotting like email, address and avatar.</div>
    <div>- A linear correlation is clearly seen between 'Length of membership' and 'Year amount spent'.</div>
    <div>- A linear correlation is seen beween 'Time on App' and 'Year amount spent', but it's less evident than 'Length of membership'</div>
  </DefaultDiv>


  <NumberTitle> 2. Make a heatmap of numerical variables.</NumberTitle>
  <CodeBlock language="python" text={heapMapCode}/>
  <ChartImg src={commerceHeapMap} style={{ height: 600 }}/>
  <DefaultDiv>
  ...
  <NumberTitle> 3. Make a linear regression model following Pareto principle.</NumberTitle>
  <CodeBlock language="python" text={linearCode}/>
  <ChartImg src={linearModel} style={{ width: 500, height: 150 }}/>
  <DefaultDiv>
    <div>Above code made a linear regrssion model utilizing pareto principle </div>
    <div>: 80% of the data will be used for training the model and 20% will be used for evaluation. </div>
  </DefaultDiv>
  
  <NumberTitle> 4. Evalute the model using RSME and predict new customers' yearly amount spent.</NumberTitle>
